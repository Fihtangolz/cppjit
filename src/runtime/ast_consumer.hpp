                ...
                if(binary_op = boost::get<ast::binary_op_t*>(statement)){
                    try{
                        if(identifier = boost::get<ast::identifier_t*>(binary_op->lhs)){
                            ...
                            //TODO: opt simple stm like a = 3;
                            // switch(binary_op->type){
                            //     case ast::binary_op_t::binary_t::basic:
                            //         represent_memreg<int>(it) = boost::get<ast::literal_t>(binary_op->rhs);
                            //     default:
                            //         break;
                            // }
                            [](ast::expression_t expr){
                                ...
                                do{
                                    if(ast::binary_op_t* b_op = boost::get<ast::binary_op_t*>(expr)){
                                        que.push(b_op->lhs);
                                        que.push(b_op->rhs);
                                    }
